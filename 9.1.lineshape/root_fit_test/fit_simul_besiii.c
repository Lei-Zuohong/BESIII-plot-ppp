...
double yv[1] = {0.0};
double ye[1] = {0.0};
bes_data::DATA usedata_pipipi(xv, xe, yv, ye, 1);
bes_data::DATA usedata_rhopi(xv, xe, yv, ye, 1);
...
double Func_rhopi(double *var, double *par)
{
    double e = var[0];
    double output = bes_func::snd_pipipi_line_shape(e,
                                                    1.019461, 0.004249, par[0], 3.1415926 * 163 / 180,
                                                    par[1], par[2], par[13], 3.1415926,
                                                    par[4], par[5], par[14], 0.0,
                                                    par[7], par[8], par[15], par[10],
                                                    par[16], par[17]);
    return output;
}
...
double func_rhopi(double x, double *par)
{
    double *input_var = new double[1];
    input_var[0] = x;
    double output = Func_rhopi(input_var, par);
    return output;
}
...
void fcn(Int_t &npar, Double_t *gin, Double_t &f, Double_t *par, Int_t iflag)
{
    Double_t chisq = 0;
    Double_t delta;
    for (Int_t i = 0; i < usedata_pipipi.ndata; i++)
    {
        delta = (usedata_pipipi.yv[i] - func_pipipi(usedata_pipipi.xv[i], par)) / usedata_pipipi.ye[i];
        chisq += delta * delta;
    }
    for (Int_t i = 0; i < usedata_rhopi.ndata; i++)
    {
        delta = (usedata_rhopi.yv[i] - func_rhopi(usedata_rhopi.xv[i], par)) / usedata_rhopi.ye[i];
        chisq += delta * delta;
    }
    f = chisq;
}

int fit_simul_besiii()
{
    usedata_pipipi = bes_data::pipipi_bes;
    usedata_rhopi = bes_data::rhopi_bes;
...
#pragma region 3.1.fit
    ...
    my_fit->mnparm(9, "b1b2_omega_new       ", 10.0000, 0.0100, 0.0000, 1000, fit_error);
    ...
    my_fit->mnparm(13, "rhopi_b1b2_omega1450 ", 1.12192e+02, 0.1000, 0.0000, 10000, fit_error);
    my_fit->mnparm(14, "rhopi_b1b2_omega1680 ", 7.66721e+02, 0.1000, 0.0000, 10000, fit_error);
    my_fit->mnparm(15, "rhopi_b1b2_omega_new ", 10.0000, 0.0100, 0.0000, 1000, fit_error);

    my_fit->mnparm(16, "rhopi_back           ", 2.83825e+01, 0.0010, 0.0000, 10000, fit_error);
    my_fit->mnparm(17, "rhopi_phase_back     ", -2.3177e+00, 0.0010, -4.000, 4.000, fit_error);
    fit_index[0] = 1000;
    fit_index[1] = 0.001;
...
#pragma region 3.3.draw
    ...
    {
        TGraph *Ddata = new TGraphAsymmErrors(usedata_pipipi.ndata,
                                              usedata_pipipi.xv,
                                              usedata_pipipi.yv,
                                              usedata_pipipi.xe,
                                              usedata_pipipi.xe,
    ...
    if (option_draw_best_function == 1)
    {
        ...
        double func_best_start[18] = {bestPar[0], bestPar[1], bestPar[2], bestPar[3], bestPar[4],
                                      bestPar[5], bestPar[6], bestPar[7], bestPar[8], bestPar[9],
                                      bestPar[10], bestPar[11], bestPar[12], bestPar[13], bestPar[14],
                                      bestPar[15], bestPar[16], bestPar[17]};
    ...
    legend->Draw();
    c1->Update();
    c1->Print("fit_simul_besiii.pdf");
    c1->Print("fit_simul_besiii.jpg");
#pragma endregion
