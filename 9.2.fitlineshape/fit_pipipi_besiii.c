...
double Func_pipipi(double *var, double *par)
{
    ...
    double output = bes_func::snd_pipipi_line_shape(e,
                                                    1.019461, 0.004249, 0.0, 3.1415926 * 163 / 180,
                                                    1.410, par[0], par[1], 3.1415926,
                                                    1.670, par[2], par[3], 0.0,
...
#pragma region 3.1.fit
    ...
    // 设定初始值、边界、步长
    my_fit->mnparm(0, "wr_omega1450         ", 9.99059e-01, 0.0010, 0.1000, 0.500, fit_error);
    my_fit->mnparm(1, "b1b2_omega1450       ", 1.12192e+02, 0.1000, 0.0000, 10000, fit_error);

    my_fit->mnparm(2, "wr_omega1680         ", 2.36609e-01, 0.0010, 0.2500, 0.400, fit_error);
    my_fit->mnparm(3, "b1b2_omega1680       ", 7.66721e+02, 0.1000, 0.0000, 10000, fit_error);

    my_fit->mnparm(4, "back                 ", 2.83825e+01, 0.0010, 0.0000, 10000, fit_error);
    my_fit->mnparm(5, "phase_back           ", -2.3177e+00, 0.0010, -7.000, 7.000, fit_error);

    my_fit->mnparm(6, "mr_omega_new         ", 2.20000, 0.0010, 2.0000, 2.400, fit_error);
    my_fit->mnparm(7, "wr_omega_new         ", 0.10000, 0.0010, 0.0100, 0.500, fit_error);
    my_fit->mnparm(8, "b1b2_omega_new       ", 1.50000, 0.0100, 0.0000, 100.0, fit_error);
    my_fit->mnparm(9, "phase_omega_new      ", 1.57000, 0.0010, -7.000, 7.000, fit_error);
...
#pragma region 3.3.draw
    ...
    if (option_draw_best_function == 1)
    {
        ...
        double func_best_start[10] = {bestPar[0], bestPar[1], bestPar[2], bestPar[3], bestPar[4],
