...
def effect(energy):
    ...
    alldata_truth = hnew.ALLDATA(trees=trees_truth,
                                 selecters=selecters,
                                 massages=massages)
    alldata_fit4c.get_weighter(data='pppmpz',
                               energy=energy,
                               name_weight='pipm_m_pipz_m_piz_a',
                               name_branch='pwa_weight')
    alldata_truth.get_weighter(data='pppmpz',
                               energy=energy,
                               name_weight='pipm_m_pipz_m_piz_a',
                               name_branch='pwa_weight')
    docuts = hppp.docuts()
...
hfile.pkl_dump('5.python_efficiency_factor/data/reweight3d.pkl', output_detail)
