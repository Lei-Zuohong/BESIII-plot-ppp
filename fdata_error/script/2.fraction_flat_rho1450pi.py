...
for energy in hppp.energy_sort():
    nx.append(energy)
    ny.append(1 / hfile.pkl_read('fdata_error/sta_efficiency_nominal/%s/%1.4f.pkl' % (section, energy)))
    ne.append(hfile.pkl_read('fdata_error/sta_efficiency_error/%s/%1.4f.pkl' % (section, energy)))
    check.append(1 / hfile.pkl_read('fdata_error/sta_efficiency_mean/%s/%1.4f.pkl' % (section, energy)))
nx = numpy.array(nx)
ny = numpy.array(ny)
ne = numpy.array(ne)
ne = ne * ny
...
axe.errorbar(nx, ny, yerr=ne,
             fmt='bo', label='Nominal')
axe.errorbar(nx, check,
             fmt='yo', label='Mean')
