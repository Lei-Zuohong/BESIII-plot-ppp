...
def std(energy):
    ...
    section_omega782pi = []
    for i in range(10):
        fraction = hfile.pkl_read('../ppp_pwa/fdata_error/sys_masswidth_fractions/%1.4f_%02d.pkl' % (energy, i))
        weight_fit4c = hfile.pkl_read('../ppp_pwa/fdata_error/sys_masswidth_weight_fit4c/%1.4f_%02d.pkl' % (energy, i))
        weight_truth = hfile.pkl_read('../ppp_pwa/fdata_error/sys_masswidth_weight_truth/%1.4f_%02d.pkl' % (energy, i))
        eff_out, detail_out = efficiency(energy, weight_fit4c, weight_truth)
    section_total.append(1 / eff_out)
    section_rho770pi.append(fraction['rho770pi']['rho770pi'] / eff_out)
    section_rho1450pi.append(fraction['rho1450pi']['rho1450pi'] / eff_out)
    section_omega782pi.append(fraction['omega782pi']['omega782pi'] / eff_out)
    return numpy.array(section_total), numpy.array(section_rho770pi), numpy.array(section_rho1450pi), numpy.array(section_omega782pi)
...
for energy in energy_sort:
    ...
    error_total = numpy.max(error_total)
    error_rho770pi = numpy.max(error_rho770pi)
    error_rho1450pi = numpy.max(error_rho1450pi)
    error_omega782pi = numpy.max(error_omega782pi)

    hfile.pkl_dump('fdata_error/pwa_masswidth/total/%1.4f.pkl' % (energy), error_total)
    hfile.pkl_dump('fdata_error/pwa_masswidth/rho770pi/%1.4f.pkl' % (energy), error_rho770pi)
    hfile.pkl_dump('fdata_error/pwa_masswidth/rho1450pi/%1.4f.pkl' % (energy), error_rho1450pi)
    hfile.pkl_dump('fdata_error/pwa_masswidth/omega782pi/%1.4f.pkl' % (energy), error_omega782pi)
