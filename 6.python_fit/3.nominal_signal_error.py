...
for energy in hppp.energy_sort():
    ...
    myfit.set_best_parameters(hfile.pkl_read('6.python_fit/output_signal/%1.4f_best_parameters.pkl' % (energy)))
    fit_data = myfit.do_fit_plot(**argv)
    # 返回结果
    output[energy] = fit_data['nevent']


massages = hnew.massage_read()
nominal = hfile.pkl_read('%s.pkl' % (massages['nevent']))
for energy in hppp.energy_sort():
    # if(energy != 2.125): continue
    error = abs(output[energy] - nominal[energy]['nevent']) / nominal[energy]['nevent']
    while(error < 0.001):
        error *= 10
    while(error > 0.02):
        error /= 10
    print(error)
    hfile.pkl_dump('fdata_error/npwa_fit_signal/%1.4f.pkl' % (energy), error)
